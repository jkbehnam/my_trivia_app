...
import static com.trivia.trivia.activities.Events.FragmentEventPresenter.getDateDifference;
...
public class DialogEventGame extends Dialog implements View.OnClickListener, IeventAlertview {
    Event event;
    Context context;
    ...
    public DialogEventGame(@NonNull Context context) {
        super(context);
    }
    ...
    public Dialog qrcode_reader(Context context, Event event, int position, FragmentActivity fragmentActivity, String u_id) {
        ...
        dialog.setContentView(R.layout.activity_event_game);
    ...
    public void ifpay() {
        if (checkProfileDetails()) {
            //connectToServer.regUserEvent(this, u_id, event.getE_id());
            showPaydialog();
        } else {
    ...
    public void init_items(Event event) {
        ...
        try {
            PersianDate persianDate = new PersianDate(Long.parseLong(event.getE_start_time()) * 1000);
        ...
        PersianDate persianDate = new PersianDate(Long.parseLong(event.getE_end_time()) * 1000);
        event_end.setText(persianDate.getShDay() + " " + persianDate.monthName() + " " + persianDate.getHour() + ":" + persianDate.getMinute());
        if (event.getE_game_type().equals("concurrent")) {
            event_lenght.setText(getDateDifference(Long.parseLong(event.getE_start_time()) * 1000, Long.parseLong(event.getE_end_time()) * 1000));
        } else {
            event_lenght.setText(event.getE_duration() + " ساعت");
        }
        event_members.setText("حداکثر " + event.getE_group_members() + "نفر");
    ...
    public boolean checkProfileDetails() {
        ...
        return !sd.equals("") && !sd.equals("null");
    }
    ...
    public void showPaydialog() {
        DialogPayCurrency dt = new DialogPayCurrency();
        ad = dt.init(context, "روش پرداخت را انتخاب کنید.", "1", event.getE_price() + " تومان", prof, u_id);
        ...
        event_money_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                // connectToServer.regUserEvent(DialogEventGame.this, u_id, event.getE_id());
