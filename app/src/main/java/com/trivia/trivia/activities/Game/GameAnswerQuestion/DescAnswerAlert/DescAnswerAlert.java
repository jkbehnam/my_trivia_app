...
public class DescAnswerAlert  extends BaseAnswer implements IGamesDA {
    Question question;
    Context context;
    Dialog dialog;
    ...
    // ScrollView scrollv;
    DAPresenter answerQuestionPresenter;
    ...
    public Dialog init_dialog(Context context, Question question, FragmentQuestionsPresenter fragmentQuestionsPresenter, int position) {
        ...
        answerQuestionPresenter = new DAPresenter(this, question);
        dialog = new Dialog(context, android.R.style.Theme_Holo_Light_NoActionBar);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.dialog_desc_answer);
...
back_arrow.setOnClickListener(new View.OnClickListener() {
            ...
            btn_ok.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (et_short_answer.getText().length() > 0) {
                        answerQuestionPresenter.checkDescAnswer(et_short_answer.getText().toString(), question.getQ_id(), question.getId(), pm.getUserDetails().get("u_id"));
                    } else
    ...
    public void correcAnswer(int item) {
        Toasty.success(context, "جواب برای داور ارسال شد", Toast.LENGTH_SHORT, true).show();
